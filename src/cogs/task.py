...
class TaskCog(commands.Cog):
    ...
    # --- 1. 提出物の登録コマンド ---
    @app_commands.command(name="add_task", description="提出物や課題を登録します")
    @app_commands.describe(
        subject="教科やタスク名 (例: 数学レポート)",
        date="提出期限 (例: 2/20)",
        remind_days="何日前に通知しますか？ (例: 3)"
    )
    async def add_task(self, interaction: discord.Interaction, subject: str, date: str, remind_days: int):
        deadline = self.parse_date(date)
        if deadline is None:
        ...
        self.assignments.append(new_task)
        
        # 日付順に並び替え
        self.assignments.sort(key=lambda x: x['date'])
    ...
    # --- 2. 一覧確認コマンド ---
    @app_commands.command(name="list_tasks", description="登録されている提出物の一覧を表示します")
