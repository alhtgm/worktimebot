...
# --- 模擬データ生成 (リスト形式で返す) ---
def get_mock_data_list(start_date, end_date):
    data_list = []
    current = start_date
...
# --- 統計コマンド ---
@bot.tree.command(name="stats", description="勉強時間の詳細データを数値リストで表示します")
...
async def stats(interaction: discord.Interaction, period: str, start: str = None, end: str = None):
    # 期間の決定ロジック
    ...
    # データ生成
    data = get_mock_data_list(start_date, end_date)
